...
function isPalindrome(str) {
  const reverseStr = str
    ...
    .toLowerCase()
    .replaceAll(/[^a-z]/g, '');
  return str.toLowerCase().replaceAll(/[^a-z]/g, '') === reverseStr;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i].codePointAt() >= 65 && str[i].codePointAt() <= 90) {
      const shift = (str[i].codePointAt() - 'A'.codePointAt() + 13) % 26;
      encodeStr += String.fromCodePoint('A'.codePointAt() + shift);
    } else if (str[i].codePointAt() >= 97 && str[i].codePointAt() <= 122) {
      const shift = (str[i].codePointAt() - 'a'.codePointAt() + 13) % 26;
      encodeStr += String.fromCodePoint('a'.codePointAt() + shift);
    } else {
      encodeStr += str[i];
